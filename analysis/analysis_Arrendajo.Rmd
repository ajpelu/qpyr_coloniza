...
- Ojo son unbalanced, usamos type III 
...
Plot2WayANOVA(formula = den ~ localidad * year, dataframe = garrulus)
```

```{r, eval=FALSE, echo=FALSE}
garrulus_mensual <- dfab %>%  
  mutate(month_year = format(as.Date(fecha, format="%Y-%m-%d"), format = "%Y-%m")) %>% 
  group_by(localidad, month_year) %>% 
  summarise(mean = mean(den, na.rm = TRUE), 
            se = std.error(den))
  ggplot(aes(x=))
